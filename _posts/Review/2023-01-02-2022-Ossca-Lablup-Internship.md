...
[Raft 구현에 관련된 논문](https://web.stanford.edu/~ouster/cgi-bin/papers/OngaroPhD.pdf)은 250 페이지가 넘고, 심지어 아직도 [하시코프 사의 Raft 구현체](https://github.com/hashicorp/raft)도 꾸준히 개발, 변경되고 있는 것을 보면서 `aioraft-ng`를 계속 개발하는 것이 정답이 아닐 수 있을 것 같다고 느끼게 되었다.

결론적으로, [gopy](https://github.com/go-python/gopy)를 사용해 파이썬 바인딩을 작성해 하시코프 사의 Raft 구현체를 플랫폼 별로 shared object로의 컴파일 함으로써 통해 python에서도 이용할 수 있도록 만드는 게 앞으로의 유지 보수 비용을 최대로 절감할 수 있을 것이라고 판단했다. (안타깝게도 shared object로의 크로스 컴파일은 `GOOS`, `GOARCH` 등의 환경 변수를 적당히 변경하는 것만으로는 작동하지 않았다.)
...
오픈소스 프로젝트나 커뮤니티 참여에 관심이 있는 분들, 본인이 일하고 싶은 분야를 찾아 앞으로의 커리어를 결정하고 싶거나, 해당 기술에 대한 이해도를 깊게 쌓아보고 싶은 분들 모두에게 Lablup 인턴십은 최고의 선택이 될 수 있을 것이라고 생각한다. 👍
