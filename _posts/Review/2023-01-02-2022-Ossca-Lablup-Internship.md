...
Lablup에서 오픈 소스와 개발자 커뮤니티들에 지속적으로 관심을 갖고 참여하고 있다는 점이 매력적으로 느껴졌다. 예를 들어 `Backend.ai`는 몇 년째 오픈소스 컨트리뷰션 아카데미에 참여해왔다.

그리고 새로운 기술 스택을 배워보고 싶다는 점도 있었다. 이전 실무에선 자바스크립트로 앱, 웹을 만드는 등 프론트 업무를 주로 했었는데 백엔드와 인프라 쪽 업무도 공부해보면서 진로에 대해 좀 더 생각해보고 싶다는 점도 지원 동기에 어느 정도 영향을 끼쳤다.
...
과제를 진행한 후엔 작성한 코드에 대해 검토 받는 시간을 가질 수 있었다. 단지 과제를 진행하는 것으로 끝내는 것이 아니라 직접 시니어 개발자 분께서 코드를 들여다 보시고 어떤 코드는 어떤 문제점이 있고 어떤 잠재적인 문제들이 발생할 수 있는지 꼼꼼히 설명해 주셔서 혼자 공부하는 것에서 끝나지 않고 평소와 다른 관점, 세부사항들에 대해 생각해 볼 수 있는 좋은 기회가 된 것 같다.
...
[두 번째 이슈](https://github.com/lablup/backend.ai/pull/893)도 비슷한 종류의 이슈로 `session rm` 명령어에 `--recursive` 옵션을 추가하는 작업이었다.
...
이번 이슈의 경우 이미 이전 이슈에서 개발 환경 구축 및 연동이 끝난 상태였기 때문에, 첫 번째 이슈에 비해 쉽게 진행할 수 있었다.
...
6주차엔 [Backend.AI Jail 디버깅 및 설정 파일 업데이트 자동화](https://jopemachine.github.io/2022/12/08/Jail-Debugging/) 작업을 진행해보았다. 이 부분에 대한 경험은 개인적으로 생소했던 경험이어서 글이 계속 길어지다보니 다른 글로 뺐다.
...
결론적으로, [gopy](https://github.com/go-python/gopy)를 사용해 파이썬 바인딩을 작성해 하시코프 사의 Raft 구현체를 플랫폼 별로 shared object로의 컴파일 함으로써 통해 python에서도 이용할 수 있도록 만드는 게 앞으로의 유지 보수 비용을 최대로 절감하는 방법이 될 수 있지 않을까 생각했다. (안타깝게도 shared object로의 크로스 컴파일은 `GOOS`, `GOARCH` 등의 환경 변수를 적당히 변경하는 것만으로는 작동하지 않았다.)
...
그래서 `gopy`를 이용해 다양한 시도를 해 보았는데 결론적으로 raft의 `Future` 타입 및 관련된 비동기 처리를 파이썬에서 따로 또 구현하는 게 적절하지 못하다고 판단해 하시코프 사의 Raft 구현체를 참고하면서 파이썬에 직접 Raft를 구현해보는 쪽으로 방향을 바꾸게 되었다.

이 부분에 대한 내용도 길어질 것 같아 다른 글로 다시 작성해보려고 한다.
