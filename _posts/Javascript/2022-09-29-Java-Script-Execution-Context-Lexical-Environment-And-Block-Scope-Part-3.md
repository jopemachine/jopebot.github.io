---
layout: post
title: 번역 - JavaScript execution context — lexical environment and block scope
  (part 3)
...
Global scope는 전역 실행 컨텍스트이고 Function scope는 함수 실행 컨텍스트와 관련됩니다.
...
ES6에 도입된 Block scope는 두 형제와 다릅니다.

## Global scope의 예

Block scope를 이해하는 가장 쉬운 방법은 다른 두 scope와 비교하는 것입니다.

변수는 Global, Function scope 둘에서 유사하게 작동하므로 이 게시물에서는 Global scope와 Block scope의 차이에 대해서만 설명합니다.
...
위의 예를 let으로 다시 작성하여 새로운 범위인 Block scope를 도입할 수 있습니다.
...
콘솔은 두 개의 다른 값을 기록합니다. 첫 번째 사과 변수는 값 "apple"을 보유하는 반면 if 블록문 내부에서 이 값은 "banana"입니다.
...
컴파일 단계에서 undefined인 apple 변수가 Global 실행 컨텍스트에 추가됩니다.
...
다음으로 실행 단계가 시작됩니다. 첫 번째 사과 변수에는 "apple" 값이 할당됩니다.
...
if 블록 코드를 읽을 때 중첩 컴파일 단계가 발생합니다. undefined인 두 번째 apple 변수가 생성됩니다.
...
곧 "banana"라는 값이 Lexical environment에서 apple에 할당됩니다.
...
let과 var의 차이점을 더 잘 이해하기 위해 재미있는 예제에서 함께 결합해 보겠습니다.

컴파일 단계에서 undefined 상태인 apple 및 grape 변수가 variable environment에서 초기화됩니다. grape 초기화는 호이스팅 됩니다. 한편, Lexical environment에서는 banana 변수가 생성됩니다.
...
실행 단계가 시작됩니다. 사과에는 "global apple" 값이 할당되고 바나나에는 "global banana"가 할당됩니다.
...
블록 범위에서 let 및 const 변수를 봤을 때 JavaScript는 해당 변수에 대해 별도의 영역을 만듭니다. Lexical environment는 변수에 대해 스택과 같은 구조를 유지하므로 이름이 같은 변수는 서로 충돌하지 않습니다.
...
여기에서 undefined인 banana 및 orange 변수는 서로 간섭하지 않는(stand-along) 범위에 있습니다.
...
첫 번째 변수를 기록할 때 JavaScript 엔진은 먼저 Lexical environment에서 위에서 아래로 apple을 찾으려고 시도합니다. 그런 다음 Global variable environment를 검사하고 "global apple"에서 로그아웃한 apple을 찾습니다.
...
banana를 검색할 때 JavaScript 엔진은 동일한 단계를 따라 찾아서 "block banana"를 기록합니다.
...
자바스크립트가 계속 실행됩니다. Global variable environment에서 banana와 grape를 찾아 "global banana"와 "global grape"를 로그아웃합니다.
...
Scope 외에 let 및 const와 관련해 뭔가 이해하신 점이 있으십니까?

## 변수 생성, 초기화, 그리고 할당에 관한 트릭
...
콘솔은 무엇을 기록합니까? apple인가요? 또는 banana입니까?
...
* let과 const 변수는 컴파일 단계 대신 block scope에서 실행 단계에 만들어집니다.

* 이 변수들은 Lexical environment에 저장됩니다.

* 여러 block scope들은 스택 구조로 Lexical environment에 저장, 유지됩니다.

* 자바스크립트 엔진이 block scope 내의 모든 코드들을 실행시키고 나면, 관련된 let과 const 변수들은 모두 제거됩니다.
