...
Raft는 로그 사용 없이 이것을 보장하기 위해 아래 두 가지 추가적인 조치를 필요로 한다.

1. 리더는 어떤 엔트리들이 커밋되었는지에 대한 최신 정보를 갖고 있어야 한다. `Leader Completeness` 성질은 리더가 모든 커밋된 엔트리들을 갖고 있다는 것을 보장해주지만, 그 `term`의 시작에서 커밋된 엔트리가 누구인지 모를 수 있다. 이것을 알아내기 위해, 리더의 `term`으로부터 엔트리를 커밋할 필요가 있다. Raft는 이것을 각 리더가 *no-op* 엔트리 (blank *no-op* entry)를 로그의 `term`의 시작 부분에 커밋함으로써 해결한다.
