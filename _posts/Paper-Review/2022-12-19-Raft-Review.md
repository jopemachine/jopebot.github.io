...
`Cluster membership change`를 위해, 스냅샷은 로그 안에 마지막으로 포함된 인덱스의 최신 설정도 포함한다.
...
- `nextIndex[]`: 각각의 서버들에 대해 갖고 있는, 다음에 보내줘야 하는 로그 엔트리의 인덱스 값.
- `matchIndex[]`: 각각의 서버들에 대해 갖고 있는, 복제된 것으로 알려진 가장 높은 인덱스 값.
