...
어떤 리더 기반의 합의 알고리즘이든, 리더는 결과적으로 커밋된 모든 로그 엔트리들을 저장해야 한다.
...
Compactionㄹ에 대한 접근으로서, `Log-cleaning`, `Log-structured merge trees (LSM)`와 같은 방법도 사용할 수 있다. 이들은 언제나 전체 데이터 셋에 대해 작동하는 스냅샷에 비해 더 복잡한 메커니즘을 요구한다.
