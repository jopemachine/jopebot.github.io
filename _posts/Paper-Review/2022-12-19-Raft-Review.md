...
그러나 리더는 엔트리가 대다수의 서버에 저장되는 즉시, 이전 `term`으로부터의 엔트리가 커밋되었는지 결론을 내릴 수 없다.
...
스냅샷은 *Chubby*, *ZooKeeper* 등에서 사용되는, Compaction에 대한 가장 간단한 접근이다. 스냅샷은 현재 전체 시스템을 안정적 저장소의 *snapshot*에 쓰고, 스냅샷 이전까지의 전체 로그는 버려진다.
