...
로그의 초기 상태는 `Log Matching Property`를 만족한다. 그리고 위 일관성 검사에 의해 로그가 늘어날 때 마다 `Log Matching Property`가 보존된다.
...
이 정보들로, 리더는 nextIndex를 해당 `term`에 해당하는 모든 충돌 엔트리들을 건너뛰도록 조정 할 수 있다.
