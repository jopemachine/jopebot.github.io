...
Raft는 커밋된 로그 엔트리들에 안전함, 견고함을 보장하며, **커밋된 엔트리들은 최종적으로 가용한(Available) 상태 머신들에 의해 실행**된다.
...
Raft 로그 메커니즘은 서버들 간에 높은 *일관성*(*Coherency*)을 갖도록 설계되었다. 이 점은 단지 시스템의 행동을 단순화하고 예측 가능하게 만들어 줄 뿐 아니라, 안정성을 보장해주는 중요한 요인이다.
...
어떤 합의 알고리즘에서는 모든 커밋된 엔트리들을 포함하지 않는 서버를 리더로 선출하기도 한다. 불행하게도 이런 접근 방법은 상당한 복잡함을 유발하기 때문에, Raft는 이전 `term` 값에서 커밋된 모든 엔트리들이 새 리더에 존재함을 보장한다. 이것은 이런 엔트리들을 팔로워에서 리더로 옮길 필요가 없게 만들어주고 데이터의 흐름을 리더에서 팔로워의 단방향으로 흘라가게 만들어주며, 리더가 절대 기존의 엔트리들을 덮어씌우지 않게 만들어준다.
...
그러나 리더는 엔트리가 대다수의 서버에 저장되는 이전 `term` 값으로부터 엔트리가 커밋되었는지에 대해 즉시 알 수 없다.

![](/img/posts/Paper-Review/2022-12-19-Raft-Review/7.png)

위 그림은 대다수의 서버들에 로그 엔트리가 저장되어 있지만, 미래의 리더가 값을 덮어씌울 수 있는 문제를 보여준다.

이런 문제를 제거하기 위해 Raft는 이전 `term` 값에선 절대 복제본(Replica)의 갯수를 세는 방식으로 커밋하지 않는다.
