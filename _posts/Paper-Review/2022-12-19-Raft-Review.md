...
Compaction에 대한 접근으로서, log-cleaning, Log-structured merge trees와 같은 방법도 사용할 수 있다.
...
Cluster Membership change를 위해, 스냅샷은 로그안에 마지막으로 포함된 인덱스의 최신 설정도 포함한다.
...
리더는 InstallSnapshot이라는 새로운 RPC를 뒤쳐진 팔로워들에게 보내는 방식을 통해 스냅샷을 보내게 된다.
...
1. 리더는 어떤 엔트리들이 커밋되었는지에 대한 최신 정보를 갖고 있어야 한다. Leader Completeness 성질은 리더가 모든 커밋된 엔트리들을 갖고 있다는 것을 보장해주지만, 그 term의 시작에서 커밋된 엔트리가 누구인지 모를 수 있다. 이것을 알아내기 위해, 리더의 term으로 부터 엔트리를 커밋할 필요가 있다. Raft는 이것을 각 리더가 비어 있는 no-op 엔트리를 로그의 term의 시작에 커밋함으로써 해결한다.
...
2. 로그에 `prevLogTerm`과 매칭되는 `prevLogIndex`에 엔트리를 포함하고 있지 않은 경우 False를 리턴.
3. 기존의 엔트리가 새로운 엔트리와 충돌(같은 인덱스를 갖지만 `term`이 다른 경우)을 일으키는 경우, 기존의 엔트리를 지운다.
...
- `term`: 리더가 자기 자신을 업데이트 하기 위한 currentTerm.
...
4. 응답하고, 만약 done이 False인 경우 더 많은 데이터 청크를 기다림.
5. 스냅샷을 저장. 기존에 존재하는 인덱스가 작은 부분들은 모두 삭제.
6. 기존 로그 엔트리의 인덱스, term이 스냅샷의 인덱스와 term과 동일하다면 로그 마지막으로 포함된 엔트리, 해당 엔트리 뒤의 로그 엔트리들을 유지하고 응답.
7. 전체 로그를 삭제
