...
사실 리더가 오래된 로그들이 커밋되었다고 결론 내릴 수 있는 몇 가지 안전한 상황들이 더 있지만, Raft는 단순성을 위해 보수적인 접근 방법을 취한다.
...
**리더 선출은 Raft에서 가장 타이밍 크리티컬한 측면**이다. 아래 타이밍 조건을 만족해야만 안정적으로 리더를 선출하고 유지할 수 있다.
