...
- `Safety Machine Property`: 어떤 서버가 로그 엔트리를 주어진 `index`에 그 서버의 상태 머신에 적용할 경우, 다른 서버들은 같은 `index`에 다른 로그 엔트리를 적용할 수 없다.
...
이 값들은 스냅샷 바로 이후의 엔트리에 대한 `AppendEntries` 일관성 검사를 돕기 위해 저장된다. 해당 엔트리는 이전의 로그 `index`와 `term` 값을 필요로 하기 때문이다.

`Cluster membership change`를 위해, 스냅샷은 로그 안에 마지막으로 포함된 `index`의 최신 설정도 포함한다.
